        ...
        if (not var_line.empty()) {
            if (var_line[0] == 'c') {
                ...
                for (int i = 0; i < pos_h.size(); i++) {
                    if (human_parsed[pos_h[i] - 1].compare("exonic") == 0 and human_parsed[12].find(',') == string::npos) { 
                        human_gene = human_parsed[pos_h[i]];
                        int j = 0;
                        bool gene_found = false;
                        while (j < human_genes_aligned.size() and not gene_found) {
                            if (human_genes_aligned[j++].compare(human_gene) == 0) gene_found = true;
                        }
                        if (not gene_found) {
                            human_genes_aligned.push_back(human_gene);
                            for (int j = 0; j < amphi_trans_to_align.size(); j++) {
                                ...
                                string human_gene_path = HUMAN_PROT_DIR + human_gene + ".fa";
                                ...
                                if (not fin.is_open()) {
                                    ...
                                    target_file_path += human_gene + "_align.fa";
                                    string trans_line;
