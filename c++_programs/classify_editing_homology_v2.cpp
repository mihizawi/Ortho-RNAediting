...
int main (int argc, char* argv[]) {
    ...
    if (argc == 2) {
        string par(argv[1]);
        if (par == "-h") {
            cout << "Usage: " << argv[0] << "variants_homology_file amphi_transcriptome_fasta_file amphi_gtf_file" << endl;
            return 0;
        }
    }
    if (argc != 4) {
    ...
    const string CLASSIFIED_EDITING_DIR = "classified_editing/";
    ...
    while (getline(var_in, var_line)) {
        if (var_line.empty()) {
            if (human_editing.size() > 0) {
                for (int i = 0; i < amphi_trans_ids.size(); i++) {
                    ...
                    for (int g = 0; g < human_genes.size(); g++) {
                        string human_gene_id = human_genes[g];
                        string align_in_path = tissue_align_dir + current_amphi_id + "_" + human_gene_id + "_align.fa";
                        ...
                        if (not align_in.is_open()) {
                            ...
                            while (getline(align_in, align_line)) {
                                if (align_line[0] == '>') {
                                    ...
                                    if (align_line[1] == 'B' and align_line[2] == 'L') {
                                        ...
                                        vector<string> trans_id_parsed = split(transcript_parsed[1], '.');
                                        current_id = trans_id_parsed[0];
                                    }
                            ...
                            for (int j = 0; j < human_editing.size(); j++) {
                                if (human_editing[j].gene_id.compare(human_gene_id) == 0) {
                                    int order = 0;
                                    bool id_found = false;
                            ...
                            for (int j = 0; j < amphi_editing.size(); j++) {
                                if (amphi_editing[j].trans_id.compare(current_amphi_id) == 0) {
                                    int hj = 0;
                                    bool homolog_editing_found = false;
                                    while (not homolog_editing_found and hj < human_editing.size()) {
                                        if (human_editing[hj].gene_id.compare(human_gene_id) == 0) {
                                            if (amphi_editing[j].aligned_position == human_editing[hj].aligned_position) homolog_editing_found = true;
                                            else hj++;
                                        }
                                        else hj++;
                                    }
                                    if (homolog_editing_found) {
                                        if (amphi_editing[j].ori_aa == amphi_editing[j].edi_aa and human_editing[hj].ori_aa == human_editing[hj].edi_aa 
                                             and amphi_editing[j].ori_aa == human_editing[hj].ori_aa) {
                                            class_0_out << amphi_editing[j].line << '\t' << amphi_editing[j].ori_aa << amphi_editing[j].position << amphi_editing[j].edi_aa << endl;
                                            class_0_out << human_editing[hj].line << endl;
                                            ...
                                            bool id_found = false;
                                            while (not id_found and order < order_of_ids.size()) {
                                                if (order_of_ids[order].compare(human_editing[hj].trans_id) == 0) id_found = true;
                                                else order++;
                                            }
                                            if (human_editing[hj].aligned_position - 1 == aligned_fasta_lines[order*2 + 1].length()) modified_alignment_lines[order*2 + 1].insert(aligned_fasta_lines[order*2 + 1].length(), "*0*");
                                            else {
                                                int k = 0;
                                                int count = 0;
                                                while (count < human_editing[hj].position) {
                                        ...
                                        else if (amphi_editing[j].ori_aa == human_editing[hj].ori_aa and amphi_editing[j].edi_aa == human_editing[hj].edi_aa) {
                                            class_1_out << amphi_editing[j].line << '\t' << amphi_editing[j].ori_aa << amphi_editing[j].position << amphi_editing[j].edi_aa << endl;
                                            class_1_out << human_editing[hj].line << endl;
                                            ...
                                            if (human_editing[hj].aligned_position - 1 == aligned_fasta_lines[order*2 + 1].length()) modified_alignment_lines[order*2 + 1].insert(aligned_fasta_lines[order*2 + 1].length(), "*1*");
                                            else {
                                                int k = 0;
                                                int count = 0;
                                                while (count < human_editing[hj].position) {
                                            ...
                                            if (human_editing[hj].aligned_position - 1 == aligned_fasta_lines[order*2 + 1].length()) modified_alignment_lines[order*2 + 1].insert(aligned_fasta_lines[order*2 + 1].length(), "*2*");
                                            else {
                                                int k = 0;
                                                int count = 0;
                                                while (count < human_editing[hj].position) {
                                            ...
                                            if (human_editing[hj].aligned_position - 1 == aligned_fasta_lines[order*2 + 1].length()) modified_alignment_lines[order*2 + 1].insert(aligned_fasta_lines[order*2 + 1].length(), "*3*");
                                            else {
                                                int k = 0;
                                                int count = 0;
                                                while (count < human_editing[hj].position) {
                                            ...
                                            class_4_out << human_editing[hj].line << endl;
                                            ...
                                            if (human_editing[hj].aligned_position - 1 == aligned_fasta_lines[order*2 + 1].length()) modified_alignment_lines[order*2 + 1].insert(aligned_fasta_lines[order*2 + 1].length(), "*4*");
                                            else {
                                                int k = 0;
                                                int count = 0;
                                                while (count < human_editing[hj].position) {
            ...
            if (human_parsed[9].compare("exonic") == 0 and human_parsed[12].compare("UNKNOWN") != 0 and human_parsed[12].find(',') == string::npos) {
                ...
                while (not gene_found and i < human_genes.size()) {
                    gene_found = human_genes[i++].compare(n_editing.gene_id) == 0;
                }
                if (not gene_found) human_genes.push_back(n_editing.gene_id);
                vector<string> editing_parsed = split(human_parsed[12], ':');
                vector<string> trans_id_parsed = split(editing_parsed[1], '.');
                n_editing.trans_id = trans_id_parsed[0];
            ...
            else if (human_parsed[13].compare("exonic") == 0 and human_parsed[16].compare("UNKNOWN") != 0 and human_parsed[16].find(',') == string::npos) {
                editing_info n_editing;
                n_editing.line = var_line;
                n_editing.gene_id = human_parsed[14];
                bool gene_found = false;
                int i = 0;
                while (not gene_found and i < human_genes.size()) {
                    gene_found = human_genes[i++].compare(n_editing.gene_id) == 0;
                ...
                if (not gene_found) human_genes.push_back(n_editing.gene_id);
                vector<string> editing_parsed = split(human_parsed[16], ':');
                vector<string> trans_id_parsed = split(editing_parsed[1], '.');
                n_editing.trans_id = trans_id_parsed[0];                
                vector<string> aa_parsed = split(editing_parsed[4], ',');
                string aa_info = aa_parsed[0];
            ...
            else if (human_parsed[17].compare("exonic") == 0 and human_parsed[20].compare("UNKNOWN") != 0 and human_parsed[20].find(',') == string::npos) {
                editing_info n_editing;
                n_editing.line = var_line;
                n_editing.gene_id = human_parsed[18];
                bool gene_found = false;
                int i = 0;
                while (not gene_found and i < human_genes.size()) {
                    gene_found = human_genes[i++].compare(n_editing.gene_id) == 0;
                ...
                if (not gene_found) human_genes.push_back(n_editing.gene_id);
                vector<string> editing_parsed = split(human_parsed[20], ':');
                vector<string> trans_id_parsed = split(editing_parsed[1], '.');
                n_editing.trans_id = trans_id_parsed[0];
                vector<string> aa_parsed = split(editing_parsed[4], ',');
                string aa_info = aa_parsed[0];
                    ...
                    if (exon_found) {
                        ...
                        if (fas_it == amphi_trans_seqs.end()) {
                            ...
                            if (trans_pos%3 == 0) {
                                ...
                                if (first != 'A' and first != 'a') cerr << "Something went wrong, the nucleaotide in  " << n_editing.trans_id << " at position " << trans_pos << " is a " << first << ", not an A!" << endl; 
                                ...
                                if (second != 'A' and second != 'a') cerr << "Something went wrong, the nucleaotide in  " << n_editing.trans_id << " at position " << trans_pos << " is a " << second << ", not an A!" << endl; 
                                n_editing.ori_aa = translate_codon(first, second, third, genetic_code_table);
                                ...
                                if (third != 'A' and third != 'a') cerr << "Something went wrong, the nucleaotide in  " << n_editing.trans_id << " at position " << trans_pos << " is a " << third << ", not an A!" << endl; 
                                n_editing.ori_aa = translate_codon(first, second, third, genetic_code_table);
