...
int main (int argc, char* argv[]) {
    ...
    const string out_csv_path = tissue_class_dir + "editing_homology_" + tissue_name + ".csv";
    ofstream csv_out(out_csv_path.c_str());
    if (not csv_out.is_open()) {
        cerr << "could not open ccsv output file" << out_csv_path << endl;
        return 1;
    }
    csv_out << "amphioxus_scaffold,amphioxus_pos,human_chromosome,human_pos,amphioxus_aa_pos,amphioxus_original_aa,amphioxus_edited_aa,human_aa_pos,human_original_aa,human_edited_aa,homology_class,amphioxus_gene_id,amphioxus_transcript_id,amphioxu_aa_seq,human_gene_id,human_transcript_id,human_aa_seq" << endl;
                            ...
                            for (int j = 0; j < amphi_editing.size(); j++) {
                                if (amphi_editing[j].trans_id.compare(current_amphi_id) == 0) {
                                    ...
                                    for (int hjj = 0; hjj < hjs.size(); hjj++) {
                                        if (amphi_editing[j].ori_aa == amphi_editing[j].edi_aa and human_editing[hjs[hjj]].ori_aa == human_editing[hjs[hjj]].edi_aa 
                                             and amphi_editing[j].ori_aa == human_editing[hjs[hjj]].ori_aa) {
                                            string amphi_aa_seq = aligned_fasta_lines[amphi_order*2 + 1];
                                            int ali_it = 0;
                                            while (ali_it < amphi_aa_seq.length()) {
                                                if (amphi_aa_seq[ali_it] == '-') amphi_aa_seq.erase(ali_it, 1);
                                                else ali_it++;
                                            }
                                            int order = 0;
                                            bool id_found = false;
                                            ...
                                            string human_aa_seq = aligned_fasta_lines[order*2 + 1];
                                            ali_it = 0;
                                            while (ali_it < human_aa_seq.length()) {
                                                if (human_aa_seq[ali_it] == '-') human_aa_seq.erase(ali_it, 1);
                                                else ali_it++;
                                            }
                                            vector<string> amphi_parsed = split(amphi_editing[j].line, '\t');
                                            vector<string> human_parsed = split(human_editing[hjs[hjj]].line, '\t');
                                            csv_out << amphi_parsed[0] << "," << amphi_parsed[1] << "," << human_parsed[0] << "," << human_parsed[1] << "," << amphi_editing[j].position << "," << amphi_editing[j].ori_aa << "," << amphi_editing[j].edi_aa
                                            << "," << human_editing[hjs[hjj]].position << "," << human_editing[hjs[hjj]].ori_aa << "," << human_editing[hjs[hjj]].edi_aa << ",class0_syn," << amphi_editing[j].gene_id << "," << amphi_editing[j].trans_id << "," << amphi_aa_seq
                                            << "," << human_editing[hjs[hjj]].gene_id << "," << human_editing[hjs[hjj]].trans_id << "," << human_aa_seq << endl;
                                        }
                                            ...
                                            vector<string> human_parsed = split(human_editing[hjs[hjj]].line, '\t');
                                            csv_out << amphi_parsed[0] << "," << amphi_parsed[1] << "," << human_parsed[0] << "," << human_parsed[1] << "," << amphi_editing[j].position << "," << amphi_editing[j].ori_aa << "," << amphi_editing[j].edi_aa
                                            << "," << human_editing[hjs[hjj]].position << "," << human_editing[hjs[hjj]].ori_aa << "," << human_editing[hjs[hjj]].edi_aa << ",class1_EtoE," << amphi_editing[j].gene_id << "," << amphi_editing[j].trans_id << "," << amphi_aa_seq
                                            << "," << human_editing[hjs[hjj]].gene_id << "," << human_editing[hjs[hjj]].trans_id << "," << human_aa_seq << endl;
                                        }
                                            ...
                                            vector<string> human_parsed = split(human_editing[hjs[hjj]].line, '\t');
                                            csv_out << amphi_parsed[0] << "," << amphi_parsed[1] << "," << human_parsed[0] << "," << human_parsed[1] << "," << amphi_editing[j].position << "," << amphi_editing[j].ori_aa << "," << amphi_editing[j].edi_aa
                                            << "," << human_editing[hjs[hjj]].position << "," << human_editing[hjs[hjj]].ori_aa << "," << human_editing[hjs[hjj]].edi_aa << ",class2_NEtoE," << amphi_editing[j].gene_id << "," << amphi_editing[j].trans_id << "," << amphi_aa_seq
                                            << "," << human_editing[hjs[hjj]].gene_id << "," << human_editing[hjs[hjj]].trans_id << "," << human_aa_seq << endl;
                                        }
                                            ...
                                            vector<string> human_parsed = split(human_editing[hjs[hjj]].line, '\t');
                                            csv_out << amphi_parsed[0] << "," << amphi_parsed[1] << "," << human_parsed[0] << "," << human_parsed[1] << "," << amphi_editing[j].position << "," << amphi_editing[j].ori_aa << "," << amphi_editing[j].edi_aa
                                            << "," << human_editing[hjs[hjj]].position << "," << human_editing[hjs[hjj]].ori_aa << "," << human_editing[hjs[hjj]].edi_aa << ",class3_MEtoE," << amphi_editing[j].gene_id << "," << amphi_editing[j].trans_id << "," << amphi_aa_seq
                                            << "," << human_editing[hjs[hjj]].gene_id << "," << human_editing[hjs[hjj]].trans_id << "," << human_aa_seq << endl;
                                        }
                                        else {
                                            string amphi_aa_seq = aligned_fasta_lines[amphi_order*2 + 1];
                                            ...
                                            vector<string> human_parsed = split(human_editing[hjs[hjj]].line, '\t');
                                            csv_out << amphi_parsed[0] << "," << amphi_parsed[1] << "," << human_parsed[0] << "," << human_parsed[1] << "," << amphi_editing[j].position << "," << amphi_editing[j].ori_aa << "," << amphi_editing[j].edi_aa
                                            << "," << human_editing[hjs[hjj]].position << "," << human_editing[hjs[hjj]].ori_aa << "," << human_editing[hjs[hjj]].edi_aa << ",class4_NEtoNE," << amphi_editing[j].gene_id << "," << amphi_editing[j].trans_id << "," << amphi_aa_seq
                                            << "," << human_editing[hjs[hjj]].gene_id << "," << human_editing[hjs[hjj]].trans_id << "," << human_aa_seq << endl;
                                        }
                                    }
                                }
